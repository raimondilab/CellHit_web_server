...
def sq_dist_to_line(ref, grad, point):
    working = ref - point
    scale = np.dot(np.ascontiguousarray(working), np.ascontiguousarray(grad))
    working = working - grad * scale
    return np.dot(np.ascontiguousarray(working), np.ascontiguousarray(working))


@jit(float32(float32[:, :], float32[:, :], float32[:], float32[:], float32), nogil=True,nopython=True)
def adjust_s_variance(data1, data2, curcell, curvect, sigma):
    ...
    curproj = np.dot(np.ascontiguousarray(grad), np.ascontiguousarray(curcell))
    prob2 = 0.
    totalprob2 = 0.
    for samecell in data2:
        sameproj = np.dot(np.ascontiguousarray(grad), np.ascontiguousarray(samecell))
    ...
    for other in range(data1.shape[0]):
        ...
        distance1[other, 0] = np.dot(np.ascontiguousarray(grad), np.ascontiguousarray(othercell))
